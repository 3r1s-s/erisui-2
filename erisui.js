...
`)}_unescapeHtml(t){const i=document.createElement("textarea");return i.innerHTML=t,i.value}_highlight(t,i){t=t.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");const e={};let s=0;function o(r,c){const d=`__TOKEN_${c}_${s++}__`;return e[d]=`<span class="token ${c}">${r}</span>`,d}return["javascript","js","json","bash","sh","css"].includes(i)?(t=t.replace(/("(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')/g,r=>o(r,"string")),i==="bash"||i==="sh"?t=t.replace(/(#.*$)/gm,r=>o(r,"comment")):i==="css"?t=t.replace(/(\/\*[\s\S]*?\*\/)/g,r=>o(r,"comment")):t=t.replace(/(\/\/.*$|\/\*[\s\S]*?\*\/)/gm,r=>o(r,"comment"))):["html","xml"].includes(i)&&(t=t.replace(/(&lt;!--[\s\S]*?--&gt;)/g,r=>o(r,"comment")),t=t.replace(/(&lt;\/?[a-z0-9:-]+)/gi,r=>o(r,"tag"))),i==="javascript"||i==="js"||i==="json"?(t=t.replace(/\b(const|let|var|if|else|for|while|return|function|class|import|export|from|default|switch|case|break|continue|new|this|async|await)\b/g,'<span class="token keyword">$1</span>'),t=t.replace(/\b(true|false|null|undefined)\b/g,'<span class="token boolean">$1</span>'),t=t.replace(/\b([a-zA-Z0-9_]+)\s*(?=\()/g,'<span class="token function">$1</span>')):i==="bash"||i==="sh"?(t=t.replace(/\b(npm|npx|node|git|cd|ls|mkdir|rm|mv|cp|echo|cat|sudo|docker|brew|grep|curl|wget|chmod|chown|touch)\b/g,'<span class="token keyword">$1</span>'),t=t.replace(/\b(install|run|build|start|test|dev|init|clone|pull|push|commit|add|checkout|branch|merge)\b/g,'<span class="token function">$1</span>'),t=t.replace(/(\s-+[a-zA-Z0-9-]+)/g,'<span class="token attr-name">$1</span>'),t=t.replace(/(\$[A-Z0-9_]+)/g,'<span class="token number">$1</span>')):i==="html"||i==="xml"?(t=t.replace(/\s([a-z0-9:-]+)=/gi,(r,c)=>" "+o(c,"attr-name")+"="),t=t.replace(/(".*?")/g,r=>o(r,"attr-value")),t=t.replace(/&gt;/g,'<span class="token tag">&gt;</span>')):i==="css"&&(t=t.replace(/([a-z-]+)(?=:)/g,'<span class="token keyword">$1</span>')),Object.keys(e).reverse().forEach(r=>{t=t.replace(r,e[r])}),t}_onCopy(){L(this._getFormattedText());const t=this.copyBtn.querySelector("eui-icon");if(t){const i=t.getAttribute("name");t.setAttribute("name","check"),setTimeout(()=>t.setAttribute("name",i),2e3)}}disconnectedCallback(){this.copyBtn&&this.copyBtn.removeEventListener("click",this._boundCopy)}}customElements.define("eui-code",V);class J extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.setupEventListeners()}static get observedAttributes(){return["name"]}attributeChangedCallback(t,i,e){t==="name"&&(this.render(),this.setupEventListeners())}setupEventListeners(){const t=this.shadowRoot.querySelector("#nav-toggle");t&&t.addEventListener("click",()=>{window.dispatchEvent(new CustomEvent("toggle-nav"))})}render(){const t=this.getAttribute("name")||"Title";this.shadowRoot.innerHTML=`
        ...
        `}}customElements.define("eui-tab-bar",G);class Q extends HTMLElement{constructor(){super();x(this,"handleClick",()=>{const i=this.getAttribute("path");i&&f.navigate(i)});this.attachShadow({mode:"open"})}static get observedAttributes(){return["path","icon","label","badge","active","avatar-src","avatar-name"]}connectedCallback(){this.hasRendered||(this.render(),this.setupEventListeners(),this.hasRendered=!0),this.updateContent(),this.updateActiveState(),this.addEventListener("click",this.handleClick),this._onRouteSubstitute=()=>this.updateActiveState(),window.addEventListener("popstate",this._onRouteSubstitute),window.addEventListener("route-changed",this._onRouteSubstitute)}disconnectedCallback(){this.removeEventListener("click",this.handleClick),window.removeEventListener("popstate",this._onRouteSubstitute),window.removeEventListener("route-changed",this._onRouteSubstitute)}attributeChangedCallback(i,e,s){if(e!==s&&this.hasRendered){if(i==="active")return;this.updateContent(),i==="path"&&this.updateActiveState()}}updateActiveState(){let i=f.location(),e=this.getAttribute("path");i.startsWith("/")||(i="/"+i),e&&!e.startsWith("/")&&(e="/"+e),e&&i===e?this.setAttribute("active",""):this.removeAttribute("active")}setupEventListeners(){this.addEventListener("pointerdown",i=>{const e=document.createElement("span"),s=i.currentTarget,o=s.getBoundingClientRect(),r=Math.max(s.offsetWidth,s.offsetHeight),c=r/2;e.classList.add("ripple"),e.style.width=`${r}px`,e.style.height=`${r}px`,e.style.left=`${i.clientX-o.left-c}px`,e.style.top=`${i.clientY-o.top-c}px`,this.shadowRoot.appendChild(e),e.addEventListener("animationend",()=>{e.remove()})})}updateContent(){const i=this.getAttribute("icon"),e=this.getAttribute("avatar-src"),s=this.getAttribute("avatar-name"),o=this.getAttribute("label")||"",r=this.getAttribute("badge"),c=this.shadowRoot.querySelector(".badge");if(r)if(c)c.textContent=r;else{const l=document.createElement("span");l.className="badge",l.textContent=r,this.shadowRoot.appendChild(l)}else c&&c.remove();const d=this.shadowRoot.querySelector("slot[name='icon']");if(d)if(e||s){let l='<eui-avatar size="24"';s&&(l+=` name="${s}"`),l+=">",e&&(l+=`<img src="${e}" alt="${s||"Avatar"}" />`),l+="</eui-avatar>",d.innerHTML=l}else i?d.innerHTML=`<eui-icon width="20" height="20" name="${i}"></eui-icon>`:d.innerHTML="";const a=this.shadowRoot.querySelector(".label");a&&(a.textContent=o)}render(){this.shadowRoot.innerHTML=`
            ...
            <div class="inner">
                ...
                <span class="label"></span>
            <slot></slot>
        `}}customElements.define("eui-tab-item",Q),typeof window<"u"&&console.log("ErisUI loaded successfully"),h.device=R,h.haptics=H,h.icons=p,h.loadPage=q,h.router=f,h.storage=I,h.utils=Z,Object.defineProperty(h,Symbol.toStringTag,{value:"Module"})}));
