(function(h,g){typeof exports=="object"&&typeof module<"u"?g(exports):typeof define=="function"&&define.amd?define(["exports"],g):(h=typeof globalThis<"u"?globalThis:h||self,g(h.eui={}))})(this,(function(h){"use strict";var it=Object.defineProperty;var st=(h,g,p)=>g in h?it(h,g,{enumerable:!0,configurable:!0,writable:!0,value:p}):h[g]=p;var v=(h,g,p)=>st(h,typeof g!="symbol"?g+"":g,p);class g extends HTMLElement{static get observedAttributes(){return["size","border-radius","name","color"]}constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.innerHTML=`
        ...
        `}connectedCallback(){this.inputEl=this.shadowRoot.querySelector("input"),this.switchEl=this.shadowRoot.querySelector(".switch"),this.updateFromAttribute(),this.switchEl.addEventListener("click",()=>this.toggle())}toggle(){this.selected=!this.selected,x(),this.dispatchEvent(new Event("change",{bubbles:!0,composed:!0}))}updateFromAttribute(){const t=this.hasAttribute("selected");this.inputEl.checked=t,this.switchEl.classList.toggle("selected",t)}attributeChangedCallback(t,i,e){t==="selected"&&this.inputEl&&this.updateFromAttribute()}get selected(){return this.hasAttribute("selected")}set selected(t){t?this.setAttribute("selected",""):this.removeAttribute("selected")}}customElements.define("eui-switch",j);class D extends HTMLElement{constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){this.shadowRoot.innerHTML=`
            ...
            <slot></slot>
        `,this.addEventListener("pointerdown",t=>{const i=document.createElement("span"),e=t.currentTarget,s=Math.max(e.offsetWidth,e.offsetHeight);i.classList.add("ripple"),i.style.left=t.clientX-e.getBoundingClientRect().left-s/2+"px",i.style.top=t.clientY-e.getBoundingClientRect().top-s/2+"px",i.style.width=s+"px",i.style.height=s+"px",this.shadowRoot.appendChild(i),setTimeout(()=>{i.remove()},600)})}}customElements.define("eui-chip",D);const w="",f=(()=>{const n=[];function t(r,c){const u=[],b=r.replace(/:([^/]+)/g,(k,C)=>(u.push(C),"([^/]+)")).replace(/\//g,"\\/"),m=new RegExp(`^${w}${b}$`);n.push({regex:m,paramNames:u,renderFn:c})}function i(r){for(const{regex:c,paramNames:u,renderFn:b}of n){const m=r.match(c);if(m){const k={};return u.forEach((C,et)=>{k[C]=decodeURIComponent(m[et+1])}),{renderFn:b,params:k}}}return null}function e(r,c=!0){r.startsWith(w)||(r=w+(r.startsWith("/")?"":"/")+r);const u=r.split(/[?#]/)[0];if(u===window.location.pathname&&r.includes("#")){c&&history.pushState({},"",r);const m="#"+r.split("#")[1];s(m);return}const b=i(u);if(b)if(c&&history.pushState({},"",r),b.renderFn(b.params),window.dispatchEvent(new CustomEvent("route-changed",{detail:{path:r}})),r.includes("#")){const m="#"+r.split("#")[1];setTimeout(()=>s(m),100)}else{const m=document.getElementById("main");m&&m.scrollTo(0,0)}else o?(c&&history.pushState({},"",r),o()):console.warn(`No route found for ${u}`)}function s(r){if(r)try{const c=document.querySelector(r);c&&c.scrollIntoView({behavior:"smooth"})}catch{console.warn("Invalid hash:",r)}}let o=null;function a(r){o=r}function l(){history.back()}function d(){return window.location.pathname.replace(w,"")||"/"}return window.addEventListener("popstate",()=>{const r=window.location.pathname+window.location.search+window.location.hash;e(r,!1)}),window.addEventListener("click",r=>{if(r.button!==0||r.metaKey||r.altKey||r.ctrlKey||r.shiftKey)return;const c=r.target.closest("a");if(!c||c.hasAttribute("download")||c.getAttribute("target")==="_blank")return;const u=c.getAttribute("href");if(!(!u||u.startsWith("http")&&!u.startsWith(window.location.origin))){if(r.preventDefault(),u.startsWith("#")){history.pushState({},"",u),s(u);return}e(u)}}),{add:t,navigate:e,back:l,location:d,setNotFound:a}})();async function q(n){try{const t=document.getElementById("main");t.classList.add("fade-out"),await new Promise(e=>setTimeout(e,200));const i=await fetch(`/src/pages/${n}.html`).then(e=>e.text());t.innerHTML=i,t.classList.remove("fade-out"),t.classList.add("fade-in"),window.location.hash?setTimeout(()=>{try{const e=document.querySelector(window.location.hash);e&&e.scrollIntoView({behavior:"smooth"})}catch{}},100):t&&t.scrollTo(0,0),setTimeout(()=>{t.classList.remove("fade-in")},200)}catch(t){console.error("Page load failed:",t)}}class B extends HTMLElement{constructor(){super();v(this,"handleClick",()=>{const i=this.getAttribute("path");i&&f.navigate(i)});this.attachShadow({mode:"open"})}static get observedAttributes(){return["path","icon","label","badge","active","avatar-src","avatar-name"]}connectedCallback(){this.hasRendered||(this.render(),this.setupEventListeners(),this.hasRendered=!0),this.updateContent(),this.addEventListener("click",this.handleClick)}disconnectedCallback(){this.removeEventListener("click",this.handleClick)}attributeChangedCallback(i,e,s){if(e!==s&&this.hasRendered){if(i==="active")return;this.updateContent()}}setupEventListeners(){this.addEventListener("pointerdown",i=>{const e=document.createElement("span"),s=i.currentTarget,o=s.getBoundingClientRect(),a=Math.max(s.offsetWidth,s.offsetHeight),l=a/2;e.classList.add("ripple"),e.style.width=`${a}px`,e.style.height=`${a}px`,e.style.left=`${i.clientX-o.left-l}px`,e.style.top=`${i.clientY-o.top-l}px`,this.shadowRoot.appendChild(e),e.addEventListener("animationend",()=>{e.remove()})})}updateContent(){const i=this.getAttribute("icon"),e=this.getAttribute("avatar-src"),s=this.getAttribute("avatar-name"),o=this.getAttribute("label")||"",a=this.getAttribute("badge"),l=this.shadowRoot.querySelector(".badge");if(a)if(l)l.textContent=a;else{const c=document.createElement("span");c.className="badge",c.textContent=a,this.shadowRoot.appendChild(c)}else l&&l.remove();const d=this.shadowRoot.querySelector("slot[name='icon']");if(d)if(e||s){let c='<eui-avatar size="24"';s&&(c+=` name="${s}"`),c+=">",e&&(c+=`<img src="${e}" alt="${s||"Avatar"}" />`),c+="</eui-avatar>",d.innerHTML=c}else{const c=d.querySelector("eui-icon");i&&c?c.getAttribute("name")!==i&&c.setAttribute("name",i):i?d.innerHTML=`<eui-icon width="24" height="24" name="${i}"></eui-icon>`:d.innerHTML=""}const r=this.shadowRoot.querySelector(".label");r&&(r.textContent=o)}render(){this.shadowRoot.innerHTML=`
        <style>
            :host {
                position: relative;
                overflow: hidden;
        ...
        <div class="app-nav-container">
            ...
            <div class="nav-items">
                ${this.navItems.map(t=>{var s,o;if(t.type==="divider")return'<div style="height: 1px; background: var(--app-300); margin: 0.5rem 0;"></div>';const i=((s=t.avatar)==null?void 0:s.src)||"",e=((o=t.avatar)==null?void 0:o.name)||"";return`<eui-nav-item path="${t.path}" icon="${t.icon}" avatar-src="${i}" avatar-name="${e}" label="${t.label}"></eui-nav-item>`}).join("")}
            </div>
        </div>
        ...
        `}connectedCallback(){this.hd=this.shadowRoot.querySelector(".header"),this.hasAttribute("type")&&this.hd.classList.add(this.getAttribute("type"))}attributeChangedCallback(t,i,e){const s=this.shadowRoot.querySelector("h1"),o=this.shadowRoot.querySelector("p"),a=this.shadowRoot.querySelector(".img");t==="title"&&(s.textContent=e),t==="subtitle"&&(o.textContent=e),t==="img"&&(a.style.backgroundImage=e?`url(${e})`:"")}}customElements.define("eui-header",N);class P extends HTMLElement{static get observedAttributes(){return["selected"]}constructor(){super(),this.attachShadow({mode:"open"}),this.shadowRoot.innerHTML=`
            <style>
                :host {
                    display: inline-block;
        ...
        `;const e=this.shadowRoot.querySelector(i);t&&e.setAttribute("href",t),this.hasAttribute("type")&&e.classList.add(this.getAttribute("type")),this.hasAttribute("icon")&&e.classList.add("icon"),this.hasAttribute("width")&&(e.style.width=this.getAttribute("width")+"px"),this.hasAttribute("height")&&(e.style.height=this.getAttribute("height")+"px"),this.hasAttribute("border-radius")&&(e.style.borderRadius=this.getAttribute("border-radius")+"px"),e.addEventListener("pointerdown",s=>{const o=document.createElement("span"),a=s.currentTarget,l=a.getBoundingClientRect(),d=Math.max(a.offsetWidth,a.offsetHeight),r=d/2;o.classList.add("ripple"),o.style.width=`${d}px`,o.style.height=`${d}px`,o.style.left=`${s.clientX-l.left-r}px`,o.style.top=`${s.clientY-l.top-r}px`,e.appendChild(o),o.addEventListener("animationend",()=>{o.remove()})})}}v(A,"observedAttributes",["type","width","height","border-radius","icon","link","href"]),customElements.define("eui-button",A);class U extends HTMLElement{static get observedAttributes(){return["type","anchor"]}constructor(){super(),this.attachShadow({mode:"open"})}connectedCallback(){this.render(),this.hd=this.shadowRoot.querySelector(".hd"),this.hasAttribute("type")&&this.hd.classList.add(this.getAttribute("type"))}render(){this.shadowRoot.innerHTML=`
            <style>
                :host {
                    display: block;
                    position: relative;
